...
| Model farms observations by repeating calls | Every unfinished action pays a small cost, and repeating the same tool and arguments against the same board state receives an additional penalty. | Longer unfinished and repeated-inspection trajectories must score below shorter, distinct ones. |
